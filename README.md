...
They are not supposed to be mutually exclusive (example: most Structured Programming is also Imperitive).
...
- REPL. "Read Eval Print Loop". Provide a simple REPL enviroment for running snippets of code. (1 point)
...
- Allow the use of a `goto` statement to jump foward/back in execution order. (1 point)
...
- Allow the definition and calling of functions (n-argument with optoinal return). (1 point)
...
- Allow funcitons to take functions as arguments. (1 point)
...
- Allow dynamic method dyspatch (given a value of parent type, call the appropriate child-defined function based on runtime type). (1 point)
...
A value binding can then be declared to be of that interface type, and can be set to any object which fufils the interface.

The key part of protocol oriented code is that funcions can and should be defined to take interfaces rather than concrete objects.
...
- Allow methods to be defined on the protocol itself, and overriden in implementing types. (1 point)
...
or rendering documentation comments from the source as a seperate document.
